...
void *fault_handler_thread(void *arg) {
  ...
  for (;;) {
    ...
    */
    ...
    // nread = pread(fd, page, PAGE_SIZE, 
    //          (msg.arg.pagefault.address & ~(PAGE_SIZE - 1)) - base);
...
int main(int argc, char **argv)
{
  ...
  if (!cla.fsimg_path) {
    ...
    // printf("image buffer: %p, size: 0x%lx.\n", image_buffer, size);
  } else {
  ...
  */
  ...
  void *res;
  pthread_t thr;
  pthread_create(&thr, NULL, activity, (void *)mpoint);
  activity((void *)mpoint);
